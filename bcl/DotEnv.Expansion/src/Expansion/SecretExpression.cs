...
public class SecretExpression
{
    ...
    public static (SecretExpression? Result, string? Error) Parse(IList<string> args)
    {
        ...
        if (parts[0].EqualsFold(CommandName) || parts[0].EqualsFold("secrets") || parts[0].EqualsFold("secret"))
        {
            parts.RemoveAt(0);
        }
        ...
        for (var i = 0; i < parts.Count; i++)
        {
            ...
            switch (part)
            {
                ...
                case "--expires-at":
                    if (j < ceiling && DateTime.TryParse(parts[j], out var expiresAtValue))
                    {
    ...
    internal static List<string> ParseArgs(string args)
    {
